...
motorLeft(){
      /usr/bin/motor -d l -s ${1}
}

motorRight(){
      /usr/bin/motor -d r -s ${1}
}

motorUp() {
      /usr/bin/motor -d u -s ${1}
}

motorDown() {
      /usr/bin/motor -d d -s ${1}
}
...
backtoOrigin() {
    ...
    x_axis=`/usr/bin/motor -d s | grep "x:" | sed "s/x: //"`
    y_axis=`/usr/bin/motor -d s | grep "y:" | sed "s/y: //"`
...
if [ -f ${FILECAMERAPOS} ]; then
	/usr/bin/motor -d s > ${FILECAMERAPOS}
fi
