...
# in /etc/timelapse.conf

PIDFILE='/run/timelapse.pid'
TIMELAPSE_CONF='/etc/timelapse.conf'
BASE_SAVE_DIR='/DCIM/timelapse'
...
while true; do
    ...
    if [ -z "$COMPRESSION_QUALITY" ]; then
        ...
        getimage | jpegoptim -m"$COMPRESSION_QUALITY" --stdin --stdout > "$SAVE_DIR/$filename" &
    fi
